...
new_cmd("Tbufnext", function()
   require("sphynx.utils").tabuflineNext()
end, {})
...
new_cmd("Tbufclose", function()
   require("sphynx.utils").close_buffer()
end, {})
...
M.Offset = function()
   for _, win in pairs(api.nvim_tabpage_list_wins(0)) do
      if vim.bo[api.nvim_win_get_buf(win)].ft == "NvimTree" then
         return "%#NvimTreeNormal#" .. string.rep(" ", api.nvim_win_get_width(win))
      end
   end
end
...
local function add_fileInfo(name, bufnr)
   ...
   local fileInfo =  name .. " "
   local pad = (24 - #fileInfo) / 2
   return string.rep(" ", pad) .. name .. string.rep(" ", pad - 1)
end
...
M.bufferlist = function()
   ...
   for _, nr in ipairs(bufnrs or {}) do -- buf = bufnr
